...
const listener = {
  onCreate(mChid) {
    chid = mChid;
    $("#status").textContent = `${mChid}`;
    $("#videoView").classList.add("remon-local-video");
    $("#videoQualityBtns").style.display = "none!important";
    toggleBtn(false);
  ...
  onJoin() {
    ...
    $("#videoView").classList.remove("remon-local-video");
    $("#videoQualityBtns").style.display = "";
    toggleBtn(false);
  },
  onClose() {
    toggleBtn(true);
...
$.all(".js-create-btn").forEach(el =>
  el.addEventListener("click", ev => {
    console.log("app/connect");
    [$("#chidInput")].forEach(
      el => _.isEmpty(el.value) && (el.value = Date.now())
    );
    createCast($("#chidInput").value);
    ev.preventDefault();
  })
);
...
function fetchCasts() {
  ...
  console.log(
    "app/fetchCasts",
    castsFetcher.version,
    castsFetcher.config.credential
  );

  fetchCastsInterval = setInterval(async () => {
    try {
      const casts = await castsFetcher.fetchCasts();
      $("#channels").innerHTML = "";
      casts
        .filter(cast => chid.trim() != cast.id.trim())
        .forEach(cast => {
          $("#channels").innerHTML += `
        <li class="list-group-item" onClick="javascript:joinCast('${cast.id}')">
          ${cast.id}
        </li>
        `;
        });
    } catch (err) {
      console.error("app>fetchCasts", err);
    }
  }, 3000);
}
